var mraid = {
    ...
    _ex: function(p) {
        this._exp = p
        var v = { "width": window.MRAID_ENV._sx, "height": window.MRAID_ENV._sy }
        ...
        this._eventListeners["exposureChange"].forEach(f => f(p,v,vr))
        this._eventListeners["viewableChange"].forEach(f => f(true))
        if (p==100) {
             this._eventListeners["ready"].forEach(f => f())
         }
    }
};
